...
exports.updatePost = async (req, res) => {
  try {
    ...
    const [updatedCount] = await CommunityPost.update({ title, content }, { where: { community_post_id: postId } });
...
exports.scrapCommunityPost = async (req, res) => {
  const userId = req.user?.userId || req.body.user_id || 1; // ✅ body에서 user_id도 받도록 수정
